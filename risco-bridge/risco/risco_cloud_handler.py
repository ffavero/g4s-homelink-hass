...
class RiscoCloudHandler(LoggingMixin):
    ...
    def get_arm_status(self) -> AlarmState:
        ...
        if part_info.get('armedStr').strip() == "Yes":
            state = AlarmState.ARMED
        elif part_info.get('disarmedStr').strip() == "Yes":
            state = AlarmState.DISARMED
        elif part_info.get('partarmedStr').strip() == "Yes":
    ...
    def set_arm_status(self, arm_status: AlarmCommand) -> dict:
        ...
        data = {
            "type": "{}".format(arm_status.value),
            "bypassZoneId": -1,
            "passcode": self.pin_auth.pin
        }
        print(data)
